...
Наименьшая накопленная стоимость на интерфейсах в сторону корневого моста.
Наименьший идентификатор отправляющего моста (Bridge ID).
Наименьший идентификатор порта отправителя (Port ID) (сначала сравнивается приоритет порта, затем номер порта).
