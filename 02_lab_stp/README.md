...
Данная работа произведена на EVE-NG.
...
На **SW2** корневым портом является **G0/0**, так как у этого порта наименьший идентификатор порта отправителя по сравнению с портом **G0/1**, который также подключен к корневому мосту SW1.

![show_016](show_016.png)

Как мы видим приоритет порта G0/0 является 128.1. Его значение ниже значения другого порта g0/1, подглюченного к Корневому мосту - 128.2
В результате этого, STP выбрал порт G0/0 в качестве root port.
...
![show_017](show_017.png)

Тут также мы видим, что STP выбрал порт G0/2 на SW3 в качестве корнегого порта, так как его приоритет+идентификатор порта ниже значения порта G0/3, который также подключен к Корневому Мосту SW1


- #### После выбора корневого моста, какое первое значение использует протокол STP для определения выбора порта?
```
Наименьшая накопленная стоимость пути до корневого моста. 
```
- #### Если первое значение на двух портах равно, какое следующее значение использует протокол STP для определения выбора порта?
```
Если первое значение одинаково на обоих портах, протокол STP использует BID (идентификатор моста) для определения выбора порта.
```
- #### Если оба значения равны на двух портах, какое следующее значение использует протокол STP для определения выбора порта?
```
Если оба значения равны на двух портах, протокол STP использует приоритет+идентификатор порта для определения выбора порта.
```