...
В этом разделе я постараюсь объяснить, **как происходит отказоустойчивость VLAN-трафика на R28 при отказе основного uplink-канала**, а также **почему поведение отличается в зависимости от дизайна PBR**.
...
но происходит **косвенно**, через таблицу маршрутизации, а не напрямую через PBR, поэтому и не очень правельны, с точки зрения дезайна.
