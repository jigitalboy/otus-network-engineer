...
```
ip sla 1
 icmp-echo 10.250.130.12 source-interface ethernet0/1
 frequency 5
```
...
```
ip sla 2
 icmp-echo 10.250.130.14 source-interface ethernet0/0
 frequency 5
```
...
- ource-interface ethernet0/0 - указвает, через какой интерфейс производить тестирование 
...
- Пока track UP, PBR использует соответствующий next-hop
- Если track переходит в DOWN, PBR считает next-hop недоступным и:
    - если есть резервный next-hop в PBR — переключается на него
    - если резервного next-hop нет — пакет обрабатывается обычной таблицей маршрутизации (default route)
...
В этом разделе я постараюсь объяснить, **как происходит отказоустойчивость VLAN-трафика на R28 при отказе основного uplink-канала 10.250.130.12**, а также **почему поведение отличается в зависимости от дизайна PBR**.

Я сравню здесь два сценария:

-   **Сценарий A --- Best Practice PBR (fallback внутри PBR)**

-   **Сценарий B --- Текущая конфигурация (fallback через таблицу маршрутизации)**

Понимание этого различия критически важно для **предсказуемой и строго определенной отказоустойчивости**.
...
1.  IP SLA до `10.250.130.12` перестаёт отвечат
2.  `track 1` перехдит в состояние **DOWN**
...
но происходит **косвенно**, через таблицу маршрутизации, а не напрямую через PBR, поэтому и не очень правельна, с точки зрения дезайна.
